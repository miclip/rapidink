	...
	async function handleImportConfig(event: Event) {
		...
		if (file.type === 'application/pdf' || file.name.endsWith('.pdf')) {
			try {
						...
						if (namesArray && namesArray.asArray) {
							...
							for (let i = 0; i < arr.length; i += 2) {
								...
								if (name.toString && name.toString().includes('rapidink-config.json')) {
									...
									alert(`Config imported! Year updated to ${config.year}`);
									return;
								}
							}
						}
				...
				alert('No config found in PDF. This PDF may not have been generated by RapidInk.');
			} catch (err) {
				console.error('PDF import error:', err);
				alert('Could not extract config from PDF');
			}
